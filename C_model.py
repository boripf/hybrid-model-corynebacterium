...
F_glu_2_part1 = df_part1['Glucose feed [L/min]']*60
F_in_2_part1 = df_part1['Feed total [L/min]']*60
...
def objective_function_no2_part1(parameters):
    # Solve the model using the optimal parameters
    time_pred, biomass_pred, substrate_pred, co2_pred = model_no2_part1(parameters)  # Solve the model using the current parameters
    biomass = pd.concat([biomass_exp_2, pd.Series(biomass_pred)], axis=1, keys=['biomass_exp', 'biomass_pred']).dropna()
    biomass_exp_ = biomass['biomass_exp'].values
    biomass_pred_ = biomass['biomass_pred'].values
    mse_x = mean_squared_error(biomass_exp_, biomass_pred_)  # Calculate mean squared error for biomass
...
def objective_function_no2_part2(parameters):
    # Solve the model using the optimal parameters
    time_pred, biomass_pred, substrate_pred, co2_pred = model_no2_part2(parameters)  # Solve the model using the current parameters
    biomass = pd.concat([biomass_exp_2, pd.Series(biomass_pred)], axis=1, keys=['biomass_exp', 'biomass_pred']).dropna()
    biomass_exp_ = biomass['biomass_exp'].values
    biomass_pred_ = biomass['biomass_pred'].values
    mse_x = mean_squared_error(biomass_exp_, biomass_pred_)  # Calculate mean squared error for biomass
